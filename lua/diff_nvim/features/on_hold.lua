---@module 'diff_nvim.features.on_hold'
---@brief Ambient, mode-aware line-diff preview on CursorHold/CursorHoldI.
---@description
--- Ported from the host config's former `autocmds.git.line_diff_on_hold`.
...
local NS = api.nvim_create_namespace("diff_nvim_on_hold_preview")
...
local function augroup(name)
  return api.nvim_create_augroup("diff_nvim_on_hold_" .. name, { clear = true })
end
...
---Register the CursorHold/CursorHoldI line-diff preview autocmds.
---@param cfg DiffNvim.Config.OnHold
---@return nil
function M.setup(cfg)
  ...
  -- Match the host-config behaviour: lower updatetime so CursorHold fires
  -- promptly for this feature.
  ...
  api.nvim_create_autocmd(events, {
    ...
    callback = function()
      ...
      local function run()
          ...
          if ok_gs and gs.preview_hunk_inline then
            ...
            if ok_inline then
              ...
              api.nvim_create_autocmd({ "CursorMoved", "BufHidden", "InsertEnter" }, {
                ...
                desc = "[diff] Clear inline diff preview on next move",
              })
              return
            end
          end
        ...
        api.nvim_create_autocmd({ "CursorMoved", "BufHidden", "InsertEnter" }, {
          ...
          desc = "[diff] Clear previous-line preview on next move",
        })
      end
    ...
    desc = "[diff] Show line diff/previous content on CursorHold/CursorHoldI (mode-aware, throttled)",
  })
  ...
  api.nvim_create_autocmd("ModeChanged", {
    ...
    desc = "[diff] Clear/abort line diff preview when leaving allowed modes",
  })
end
