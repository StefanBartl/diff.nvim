...
| `side_by_side_spec.lua` | `view=vsplit`/`split`/`tab` honouring `source=`/a range on the left-hand side (and still using the live origin buffer for plain `source=current`), diffmode staying window-local, and buffer-number side labels resolving to the buffer's name (`buf:N` when unnamed). |
